...
class FrontEnd_Helper_TestEntitiesFunctions
{
    ...
    public function saveUser($profileId, $websiteId)
    {
        ...
        $user->profileimage = $this->entityManagerUser->find('KC\Entity\User\ProfileImage', $profileId);
        $user->deleted = 0;
        $user->addtosearch = '0';
        $user->website[] = $this->entityManagerUser->find('KC\Entity\User\Website', $websiteId);
    ...
    {
        $user = $this->entityManagerUser->find('KC\Entity\User\User', $userId);
        return $user;
    }
