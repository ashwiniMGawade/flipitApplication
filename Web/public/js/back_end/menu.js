 ...
 */ 
 $.ajax({
	    url : HOST_PATH + "admin/menu/getid",
		method : "post",
		dataType : "json",
...
function showModel(id,rootid,type){
	...
	if(id!=0){
		...
		$('input#hid').val(id);
		$.ajax({
			url : HOST_PATH + "admin/menu/getrecord/id/"+id,
			method : "post",
			dataType : "json",
				type : "post",
				success : function(data) {
					...
					$('input#label').val(htmlDecode(data[0]['name']));
...
function submitForm(){
	...
	switch(value){
	 case 'edit':
		...
		//$('form#menuForm').submit();
		 $.ajax({
				url : HOST_PATH + "admin/menu/editmenu/id/"+$('input#hid').val(),
				method : "post",
	...
	$.ajax({
				url : HOST_PATH + "admin/menu/addmenu",
				method : "post",
				data: $('form#menuForm').serialize(),		
				dataType : "json",
					type : "post",
...
function getMenu(){
	...
	//$('form#menuForm').submit();
	 $.ajax({
			url : HOST_PATH + "admin/menu/listmenu",
			method : "post",
			//data: $('form#menuForm').serialize(),		
			dataType : "json",
				//type : "post",
				...
				//alert(data[0]['name']);
					...
					if (data != null) {
						...
						var li = "<li id='"+data[i]['id']+"'><a id='"+data[i]['id']+"' href='javascript:void(0)' onclick='getrightMenus("+data[i]['id']+")' title='" + htmlDecode(data[i]['name'])+"'>"+str+"</a><a class='edit' href='javascript:void(0);' onclick='showModel("+data[i]['id']+","+data[i]['root_id']+","+'"get"'+")' >"+"<img src='"+HOST_PATH+"public/images/back_end/edit-icon-txt.png'></a></li>";
...
function getrightMenus(id){
	...
	//liselected(id);
	$.ajax({
			url : HOST_PATH + "admin/menu/getrightmenu/id/"+id,
			method : "post",
			//data: $('form#menuForm').serialize(),		
			//timeout: 20000,
			dataType : "json",
				...
				success : function(data) {
					...
					if (data != null) {
						...
						for(var i=0;i<length;i++){
							...
							if(data[i]['level']==0){}
							if(data[i]['level']==1){
								div = "<div class='m-beheer-rt-txt' id='"+data[i]['id']+"'>" +
										"<ul>" +
											"<li id='"+data[i]['id']+"_li' title='" + htmlDecode(data[i]['name']) +"'>" +
												"<font>"+str+"</font>" +
												"<a href='javascript:;' title='"+__('edit')+"' onclick='showModel("+data[i]['id']+","+data[i]['root_id']+","+'"get"'+")'>" +
													"<img src='"+HOST_PATH+"public/images/back_end/edit-icon-hdr.png'>" +
												"</a>" +
											"</li>" +
										"</ul>" +
									"</div>";
								...
								var last="<li id='lastliSub'><button type='button' class='btn btn-primary fr' title='"+__("Add")+"' onclick='showModel("+data[i]['id']+","+data[i]['root_id']+","+'"addRightli"'+");' data-loading-text='"+__("Loading...")+"'>+</button></li>";
								$('div.m-beheer-rt div.'+classdiv+' div.m-beheer-rt-txt ul:last').append(last);
							}
							
							if(data[i]['level']==2){
								
								var lirt="<li id='"+data[i]['id']+"_li' title='"+ htmlDecode(data[i]['name']) +"'>" +
											"<span class='fl'>"+str+"</span>" +
											"<a title='"+__("edit")+"' href='javascript:;' onclick='showModel("+data[i]['id']+","+data[i]['root_id']+","+'"get"'+")'>" +
												"<img src='"+HOST_PATH+"public/images/back_end/edit-icon-txt.png'>" +
											"</a>" +
										"</li>";
...
function deleteMenu(id,parentId,rootId){
	$.ajax({
		url : HOST_PATH + "admin/menu/deletemenu/id/"+id+"/parentId/"+parentId+"/rootId/"+rootId,
		method : "post",
		dataType : "json",
			success : function(data) {
				if (data != null) {
