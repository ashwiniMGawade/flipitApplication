...
class updatePageAttributeIdInPageTable
{
    ...
    public function __construct()
    {
        ...
        foreach ($databaseConnections as $databaseConnectionKey => $databaseConnection) {
            if ($databaseConnectionKey != 'imbull') {
                try {
                    $this->updatePageAttributeId($databaseConnection ['dsn'], $databaseConnectionKey, $imbull);
                } catch (Exception $e) {
                    echo $e->getMessage();
                    echo "\n\n";
                }
    ...
    protected function updatePageAttributeId($dsn, $key, $imbull)
    {
        if ($key == 'en') {
            $this->_localePath = '';
        ...
        defined('PUBLIC_PATH')  || define('PUBLIC_PATH', dirname(dirname(dirname(__FILE__))) . "/public/");
        set_time_limit(0);
        $idUpdtaed =  Page::updatePageAttributeId();
        if ($idUpdtaed) {
            echo "\n";
            print "$key - Attribute Id have been updated successfully!!!";
        } else {
            echo "\n";
            print "$key - Attribute Id have already been updated successfully!!!";
        }
        $doctrineManager->closeConnection($doctrineManagerConnection);
    }
}
