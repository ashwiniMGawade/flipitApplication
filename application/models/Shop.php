 ...
 */
 ...
 /*
  * sunny patial get store detail updated by kr
 * */
 public static function getStoredetail($shopId)
 {

 	$nowDate = date('Y-m-d 00:00:00');
 	$Q = Doctrine_Query::create()->select('s.*,img.*,scr.*,small.*,big.*')
 	->from('Shop s')
 	->leftJoin('s.logo img')
 	...
 	//->andWhere('o.endDate >'."'$nowDate'");//please uncomment if testing is success
 	//die();
 	$allStoreForCondition  = $Q->fetchArray(array(),Doctrine_Core::HYDRATE_ARRAY);
...
{
	   $flag = 0;
		$pc  = Doctrine_Query::create()->from('FavoriteShop s')
			->where('s.visitorId='.$userid)
		...
		return array('shop' => $shopname->toArray() , 'flag' => $flag);

}
/**
 * add shop in favorites
 *
 * @author kraj
 * @param integer $userid
 * @param integer $shopid
 * @return multitype:number json
 * @version 1.0
 */
public static function shopAddInFavoriteInShopDetails($userid,$shopid)
{
	$flag = 0;
	$pc  = Doctrine_Query::create()->from('FavoriteShop s')
	->where('s.visitorId='.$userid)
	...
	//return only shop name for selection or add to favorite
