...
class Visitor extends BaseVisitor
{
	...
	public static function addfrontVisitor($params){


		if(self::validateEmail(base64_decode($params['emailAddress'])))
		{
			$visitor = new Visitor(); // A Doctrine Model with timestampable behavior enabled
	    	$visitor->firstName = FrontEnd_Helper_viewHelper::sanitize($params['firstName']);
			$visitor->lastName = FrontEnd_Helper_viewHelper::sanitize($params['lastName']);
			//$visitor->username = $params['uname'];
			$visitor->gender = FrontEnd_Helper_viewHelper::sanitize($params['gender'] == 'male' ? 0 : 1);

			$visitor->email = FrontEnd_Helper_viewHelper::sanitize(base64_decode($params['emailAddress']));

			$visitor->dateOfBirth = ($params['birthYear'].'-'.$params['birthMonth'].'-'.$params['birthDay']);
			$visitor->postalCode = FrontEnd_Helper_viewHelper::sanitize(isset($params['postCode']) ? $params['postCode'] : '');
			$visitor->password = FrontEnd_Helper_viewHelper::sanitize(md5($params['confirmPassword']));
