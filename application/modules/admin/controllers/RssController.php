...
class Admin_RssController extends Zend_Controller_Action {
	...
	public function  newOffersAction()
	{
		...
		$feedData = array(
				'title'=> $this->view->translate('Newest offers') ,
				'link'=> $domainPath ,
				'charset'=>'UTF-8',
				'entries'=>$entries
		);
		...
		$fileName = $this->view->translate('newest-offers');
		...
		$message = $this->view->translate('RSS feed  for newest offers has been created successfully!!!');
		$flash = $this->_helper->getHelper('FlashMessenger');
	...
	public function  popularOffersAction()
	{
		...
		// Create the RSS array
		$feedData = array(
			'title'=> $this->view->translate('Popular offers') ,
			'link'=> $domainPath ,
			'charset'=>'UTF-8',
			'entries'=>$entries
			);
			...
			# generate translated file name
			$fileName = $this->view->translate('popular-offers');
			...
			$message = $this->view->translate('RSS feed  for popular offers has been created successfully!!!');
			$flash = $this->_helper->getHelper('FlashMessenger');
			$flash->addMessage(array('success' => $message));
			$this->_helper->redirector(index , 'rss' , null ) ;
