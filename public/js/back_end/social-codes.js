...
function init() {
    ...
    jQuery('div.multiselect ul li').click(selectPagesInList);
    jQuery('.close').hide();    
    ...
    jQuery("form").submit(function() {
        ...
        if (jQuery("input[name=printableCheckbox]").is(":checked") 
            && jQuery("input[name=uploadoffercheck]").is(":checked")
        ) {           
...
function showNotExistDiv()
{
    ...
    jQuery("input#notExistingShopCheckbox").attr('checked' , 'checked');
...
function showExistDiv()
{
    ...
    jQuery("input#existingShopCheckbox").attr('checked' , 'checked');
...
{   
    ...
    if (jQuery('#euro').hasClass('btn-primary')) {
        jQuery("#addofferTitle, #extendedOfferTitle, #extendedOfferRefurl")
        .val(__('At the value of ') + ' \u20AC'+discountValue+ __(' at ') +' '+shopName);
...
function setFormData(data) {
    ...
    if (data[0].Visability!=undefined && data[0].Visability == 'MEM') {
    ...
    if (data[0].discountType=='CD') {
        ...
        if (data[0].extendedOffer==true) {
            ...
            if (data[0].extendedTitle != null && data[0].extendedTitle != '') {
                ...
                jQuery('#metaTitleLeft')
                .val(__("Extended offer meta title length ") 
                    + parseInt(data[0].extendedTitle.length) + __(" characters"));
            ...
            if (data[0].extendedTitle != null && data[0].extendedTitle != '') {
                // alert(parseInt(data[0].extendedTitle.length));
                 ...
                 jQuery('#extendedTitleLeft')
                 .val(__("Extended title length ") + parseInt(data[0].extendedTitle.length) + __(" characters"));
            }
            if (data[0].extendedMetaDescription != null && data[0].extendedMetaDescription != '') {
                 ...
                 jQuery('#metaDescLeft')
                 .val(__("Extended offer meta description length ") 
                    + parseInt(data[0].extendedMetaDescription.length) + __(" characters"));
...
function selectOfferType(dIv) {
    ...
    switch(dIv) {
        case 'default':
            ...
            jQuery("input#defaultoffercheckbox").attr('checked' , 'checked');
        ...
        case 'memberOnly':
            ...
            jQuery("input#memberonlycheckbox").attr('checked' , 'checked');
...
function selectDiscountType(dIv) {
    ...
    switch(dIv) {
        case 'couponCode':
        jQuery("input[name=couponCode]").parent("div").removeClass("error success focus").prev("div").html("");
        jQuery('#couponDiv').show();
        jQuery('#printDiv').hide();
        jQuery('span#saleDiv').hide();
        jQuery('#newsDiv').hide();
        ...
        jQuery("input#couponCodeCheckbox").attr('checked' , 'checked');   
        jQuery("input#newsCheckbox").removeAttr('checked');          
        jQuery("input#saleCheckbox").removeAttr('checked');          
        ...
        case 'news':
            ...
            jQuery('#offerrefurlPR').val('');
            jQuery('#uploadoffer').val('');
            jQuery("input#newsCheckbox").attr('checked' , 'checked');   
            jQuery("input#saleCheckbox").removeAttr('checked');    
            jQuery("input#couponCodeCheckbox").removeAttr('checked');    
            jQuery("input#printableCheckbox").removeAttr('checked');     
        ...
        case 'sale':
            ...
            jQuery('#offerrefurl').val('');
            jQuery('#uploadoffer').val('');
            jQuery("input#saleCheckbox").attr('checked' , 'checked');   
            jQuery("input#newsCheckbox").removeAttr('checked');          
            jQuery("input#couponCodeCheckbox").removeAttr('checked');     
            jQuery("input#printableCheckbox").removeAttr('checked'); 
        ...
        case 'printable':
            ...
            jQuery("#attachpagesDiv").show();
            jQuery('#extra-options').hide();
            jQuery("input#printableCheckbox").attr('checked' , 'checked');   
            jQuery("input#newsCheckbox").removeAttr('checked');         
            jQuery("input#saleCheckbox").removeAttr('checked');         
            jQuery("input#couponCodeCheckbox").removeAttr('checked');     
            jQuery("#extendedNo").click();
...
function printOption(dIv) {
    ...
    switch(dIv) {
        ...
        jQuery('#uploadofferDiv').show();      
        jQuery('#offerrefurlDiv').hide();       
        jQuery('#offerrefurlPR').val(''); 
        jQuery("input#uploadoffercheck").attr('checked' , 'checked');     
        jQuery("input#offerrefurlcheck").removeAttr('checked'); 
        jQuery("#offerLogoId").show();
        break;
        case 'userefurlOption':
          jQuery('#offerrefurlDiv').show();           
          jQuery('#uploadofferDiv').hide();           
          jQuery('#uploadoffer').val('');              
          jQuery("input#offerrefurlcheck").attr('checked' , 'checked');       
...
function addMoreTerms() {
    ...
    var options = {'maxCharacterSize': 80, 'displayFormat' : ''};
...
function validateFormAddNewOffer() {
    ...
    .validate({
        ...
        rules : {
            ...
            spancombo : { 
                required : function(el) {
                    if (jQuery("input[name=defaultoffercheckbox]").is(":checked") ) 
                        return true ;
                     else 
                        return false ;
                }
            ...
            couponCode : { 
                required : function(el) {
                    if (  jQuery("input[name=couponCodeCheckbox]").is(":checked") ) {
        ...
        onfocusin : function(element) {
            ...
            if (!jQuery(element).parent('div').prev("div").hasClass('success')) {
        ...
        highlight : function(element, errorClass, validClass) {  
            ...
            jQuery('span.help-inline', jQuery(element).parent('div').prev('div')).removeClass(validClass);
        ...
        unhighlight : function(element, errorClass, validClass) {
            ...
            if (! jQuery(element).hasClass("ignore")) {
                ...
                jQuery('span.help-inline', jQuery(element).parent('div').prev('div')).text(validRules[element.name]);
            } else {
                var showError = false ;
                ...
                if (! showError) {
                    jQuery('span.help-inline', jQuery(element).parent('div').prev('div')).hide();
                    jQuery(element).parent('div')
                    .removeClass(errorClass)
                    .removeClass(validClass)
                    .prev("div")
        ...
        success: function(label , element) {
            ...
            .addClass(this.validClass).prev("div").addClass(this.validClass)
            .removeClass(this.errorClass);
...
function validateStartEndTimestamp()
{
    ...
    if (hasError) {
        jQuery("div.dateValidationMessage1").removeClass("success").addClass("error")
        .html(__("<span class='error help-inline'>End date should be greater than start date</span>"))
        .next("div").addClass("error").removeClass("success");
        jQuery("div.dateValidationMessage2").removeClass("success").addClass("error")
        .html(__("<span class='error help-inline'></span>"))
        .next("div").addClass("error").removeClass("success");
        ...
        success : function(data) {
            if (data != null) {
                ...
                if (parseInt(jQuery('input#forDelete').val()) > 0) {
                    ...
                    switch(divVal) {
                        case 'sale':
                            jQuery('div#offertypeDiv span#saleDiv div#offertilesimg')
                            .children('ul#imgLiForTiles').children('li#'+ Id).replaceWith(li);
                            break;
                        case 'printable':
                            jQuery('div#offertypeDiv span#printDiv div#offertilesimg')
                            .children('ul#imgLiForTiles').children('li#'+ Id).replaceWith(li);
                            break;
                        case 'couponCode':
                            jQuery('div#offertypeDiv span#couponDiv div#offertilesimg')
                            .children('ul#imgLiForTiles').children('li#'+ Id).replaceWith(li);
                            break;
                        default:
                        break;
                    ...
                    case 'sale':
                        jQuery('div#offertypeDiv span#saleDiv div#offertilesimg')
                        .children('ul#imgLiForTiles').children('li#before').before(li);
                        break;
                    case 'printable':
                        jQuery('div#offertypeDiv span#printDiv div#offertilesimg')
                        .children('ul#imgLiForTiles').children('li#before').before(li);
...
function selectOfferImage(id)
{
    ...
    if (jQuery("#couponCodeCheckbox").is(":checked")) {
        ...
        jQuery("input#selectedTiles").attr('checked' , 'checked');
    }
    
    if (jQuery("#saleCheckbox").is(":checked")) {
        jQuery("span[for='saleTiles[]']").remove();
        jQuery("input#saleTiles").attr('checked' , 'checked');
    }
}
