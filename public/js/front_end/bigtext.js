var BigText = {
    ...
    {
        return $('<a/>').attr('id', id).html('<style>' + css.join('\n') + '</style>');
    },
    generateFontSizeCss: function(linesFontSizes, lineWordSpacings)
    {
        ...
        for(var j=0, k=linesFontSizes.length; j<k; j++) {
            css.push('.' + BigText.LINE_CLASS_PREFIX + j + ' { font-size: ' + linesFontSizes[j] + 'em !important;'+ (lineWordSpacings[j] ? ' word-spacing: ' + lineWordSpacings[j] + 'px;' : '') + ' }');
        }
...
$.fn.bigtext = function(options)
{
    ...
    return this.each(function()
    {
        ...
        $t.find('> a').each(function(lineNumber)
        {
            $(this).each(function()
                {
